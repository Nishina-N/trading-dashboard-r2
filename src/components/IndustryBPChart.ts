...
export function createIndustryBPChart(
  ...
  const top10Industries = latestData
    .sort((a, b) => (b.bp_raw || 0) - (a.bp_raw || 0))
    .slice(0, 10)
    .map(d => d.industry!);
  ...
  const rankings = top10Industries.map((industry, index) => {
    ...
    return {
      name: industry,
      color: COLORS[index % COLORS.length],
      latestValue: industryData[industryData.length - 1]?.value || 0,
    };
  });
  ...
  rankings
    ...
    .forEach(item => {
      ...
      legendItem.innerHTML = `
        ...
        <div class="legend-rank">${item.latestValue.toFixed(3)}</div>
      `;
